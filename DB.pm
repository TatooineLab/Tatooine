...
sub _serializeCondition {
    ...
    foreach (keys %{$sql->{$field}}) {
        ...
        if ( ref $sql->{$field}{$_} eq 'HASH' and $sql->{$field}{$_}{tsearch} and $sql->{$field}{$_}{sign} ){
            my $val = join(' '.$sql->{$field}{$_}{sign}.' ', split(/\s+/, $sql->{$field}{$_}{value}));
            my $key = $_;
            ...
            my $dictionary = 'english';
            $dictionary = $sql->{$field}{$_}{dictinary} if $sql->{$field}{$_}{dictinary};
            $dictionary = $sql->{$field}{$_}{dictionary} if $sql->{$field}{$_}{dictionary};
            ...
            # Объединение
                ...
                foreach my $i (@{$val}) {
                    ...
                    # Полнотекстовый поиск
                        ...
                        my $ts_v = join(' '.$s.' ', split(/\s+/, $v));
                        my $dictionary = $i->{dictionary} || $i->{dictinary} || 'english';
                        push @tmp, "to_tsvector('$dictionary', $k) @@ to_tsquery(?)" ;
                        push @bind_values, $ts_v;
                    } else {
                        push @tmp, $k." ".$s." ?";
                        push @bind_values, $v;
                    }
